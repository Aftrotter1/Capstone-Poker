...
import random
from pokerhands import evaluate_hand, from_straight
...
class SklanskySys2(Strategy):
	...
	def decide_play(self, player, pot, log):
		...
		hand_value, rep=player.get_value()
		raw_data = ([], 0, 0, 0)
	...
	def decide_play(self, player, pot, log):
		...
		if len(player.total_cards) == 2:
			...
			if vals[0] == vals[1]:
				...
				dist = abs(vals[0] - vals[1])
				score = max(0, 5 - dist)*12
				score += 2*max(vals)
		elif len(player.total_cards) == 7:
			score = player.get_value()[0] * 50
		else:
			score = player.get_value()[0] * 50
			if player.get_value()[0] < 4:
				if from_straight(player.total_cards) < min(from_straight(player.table_cards), 2):
					score += 50

		
		norm = min(player.to_play / float(player.stack), 1.0)
		thresh = 1 - score/275.0 + norm/5 - min(wealth, 20)/100
